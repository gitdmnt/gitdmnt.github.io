...
interface Props {
  allTags: PortfolioTags[];
}

export const AddEntryForm: React.FC<Props> = ({ allTags }) => {
  ...
  const inputCls =
    "w-full bg-white/5 border border-white/15 rounded-lg px-3 py-2 text-sm placeholder-white/30 focus:outline-none focus:border-white/40 transition-colors text-white";
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                className={status === "" ? inputCls + "/40" : inputCls}
                value={status}
                onChange={(e) => setStatus(e.target.value)}
              >
                {/* プレスホルダー風の項目は薄い色にしておく */}
                <option value="" className="text-black">
                  選択なし
                </option>
                {/* 実際の選択肢はドロップダウンで黒文字にして背景が白でも見えるように */}
                <option value="運用中" className="text-black">
                  運用中
                </option>
                <option value="beta" className="text-black">
                  beta
                </option>
                <option value="開発中" className="text-black">
                  開発中
                </option>
                <option value="絶版" className="text-black">
                  絶版
                </option>
                <option value="在庫あり" className="text-black">
                  在庫あり
                </option>
              </select>
            </div>
